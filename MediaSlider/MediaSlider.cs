...
namespace MediaSlider
{
    ...
    public partial class MediaSlider : UserControl
    {
        ...
        #region Fields
        private Font _flyoutFont = new Font("Arial", 8f, FontStyle.Regular);
        ...
        private bool _bInitializeAlways = false;
        ...
        private int _iFlyOutLetterSpace = -1;
        private double _dButtonPosition = 0;
        private float _fAnimationSize = .1f;
        private int _firstHighlightedFrame = -1;
        private int _lastHighlightedFrame = -1;
        ...
        private Color _clrHighlight = Color.Red;
        ...
        [Description("Raised when a marker value has changed.")]
        public event EventHandler MarkerValueChanged;

        /// <summary>
        /// Fires the <see cref="MarkerValueChanged"/> event.
        /// </summary>
        private void onMarkerValueChanged()
        {
            if(MarkerValueChanged != null)
                MarkerValueChanged(this, new EventArgs());
        }
        #endregion
        ...
        /// <summary>Number of characters reserved in the flyout size.</summary>
        [Browsable(true), Category("Appearence"),
        Description(@"Defines the size of the flyout. If set to -1, the size
            will be calculated from the specified number range.")]
        public int FlyOutLetterNumber
        {
            get { return _iFlyOutLetterSpace; }
            set
            {    
                _iFlyOutLetterSpace = value;
                calculateFlyoutSize();
        ...
        /// <summary>First frame that will appear as highlighted.</summary>
        [Browsable(false),
        Description("First frame that will appear as highlighted.")]
        public int FirstHighlightedFrame
        {
            get { return _firstHighlightedFrame; }
            set
            {
                _firstHighlightedFrame = value;
                PropertyChange();
                onMarkerValueChanged();
            }
        }

        /// <summary>Last frame that will appear as highlighted.</summary>
        [Browsable(false),
        Description("Last frame that will appear as highlighted.")]
        public int LastHighlightedFrame
        {
            get { return _lastHighlightedFrame; }
            set
            {
                _lastHighlightedFrame = value;
                PropertyChange();
                onMarkerValueChanged();
            }
        }

        /// <summary>Whether to display the highlight bar.</summary>
        [Browsable(true), Category("Appearance"),
        Description(@"Whether to display the highlight bar if the 
LastHighlightedFrame and FirstHighlightedFrame properties 
are set to values in range.")]
        ...
        /// <summary>Modify highlighting color.</summary>
        [Browsable(true), Category("Appearence"),
        Description("Modify highlighting color")]
        public Color HighlightColor
        {
            get { return _clrHighlight; }
            set
            {
                _clrHighlight = value;
                PropertyChange();
        ...
        /// <summary>Initialize indipendant of visibility.</summary>
        [Browsable(true), Category("Appearence"),
        Description("Initializes the control even if its not visible.")]
        public bool InitializeAlways
        {
            get
            {
                return _bInitializeAlways;
            }
            set { _bInitializeAlways = value; }
        }
        ...
        public bool IsInited
        {
            get {
                if(InitializeAlways)
                    ...
                    return this.Visible && _bPropInit;
            }
            private set {
                if(InitializeAlways)
                    _bPropInit = value;
                else
                    _bPropInit = this.Visible && value;
            }
        }
        ...
        private void calculateFlyoutSize()
        {
            if (_eFlyOutOnFocus != FlyOutStyle.None)
            {
                if (Orientation == Orientation.Horizontal)
                {
                    ...
                    this.FlyOutSpacer = 6;
                    if (FlyOutLetterNumber != -1)
                    {
                        StringBuilder constructor = new StringBuilder();

                        for (int i = 0; i < FlyOutLetterNumber; i++)
                            constructor.Append("0");

                        if (Minimum < 0)
                            constructor.Append("-1");

                        String maximum_text_string = constructor.ToString();
                        SizeF textSizeMax = TextRenderer.MeasureText(
                                            maximum_text_string, _flyoutFont);
                        FlyOutMaxWidth = (int)textSizeMax.Width + 5;
                        TrackPadding = (int)(FlyOutMaxWidth / 2);
        ...
        private System.Object VALUELOCK = new System.Object();
        private bool usedKeys;
        private bool usedMouse;
        private MouseEventArgs eventMouse;
        private Keys eventKeys;
        ...
        public int Value
        {
            ...
            set
            {
                ...
                try
                {
                    ...
                    else if (!this.DesignMode)
                    {
                        ...
                        lock (VALUELOCK)
                        {
                            if (ValueChanged != null)
                            {
                                ...
                                {
                                    usedKeys = false;
                                    ValueChanged(this, new KeyEventArgs(eventKeys));
                                }
                                else
                                    if (usedMouse)
                                    {
                                        usedMouse = false;
                                        ValueChanged(this, eventMouse);
                                    }
                                    else
                    ...
                    if(ErrorHandler != null)
                        this.ErrorHandler.SetError(this, ex.Message); }
            }
        }
        #endregion
        #endregion
            ...
            lock (VALUELOCK)
            {
                ...
                switch (keyData)
                {
                    ...
                    case Keys.PageDown:
                        ...
                        return true;
                    case Keys.Home:
                        ScrollChange(ChangeType.Maximum, true);
                        DrawSlider();
                        return true;
                    case Keys.End:
                        ScrollChange(ChangeType.Maximum, false);
                        DrawSlider();
                        return true;
                }
                usedKeys = false;
        ...
        private void DrawTicks(Graphics g, Rectangle bounds)
        {
            ...
            #region Draw Highlight Tick(s)
            if ( false && ShowHighlight &&
                ((FirstHighlightedFrame <= Maximum && FirstHighlightedFrame >= Minimum) ^
                ((LastHighlightedFrame <= Maximum && LastHighlightedFrame >= Minimum))))
            {
                List<int> to_mark = new List<int>(2);
                if (FirstHighlightedFrame >= Minimum && FirstHighlightedFrame <= Maximum)
                    to_mark.Add(FirstHighlightedFrame);
                if (LastHighlightedFrame >= Minimum && LastHighlightedFrame <= Maximum)
                    to_mark.Add(LastHighlightedFrame);
                ...
                using (GraphicsMode md = new GraphicsMode(g, SmoothingMode.None))
                {
                    if (this.Orientation == Orientation.Horizontal)
                    {
                        ...
                        using (Pen pn2 = new Pen(this.HighlightColor, 1f), pn3 = new Pen(Color.FromArgb(100, Color.DarkGray), 1f))
                        {
                            foreach (int i in to_mark)
                            {
                                val = (increment * i) + offset;
        ...
        private void CreateTrack()
        {
            Rectangle bounds = new Rectangle(0, 0, this.Width, this.Height);
            Rectangle trackRect = GetTrackRectangle();
            Rectangle highlightRect = GetTrackRectangle();
            Rectangle buttonRect = GetButtonRectangle();
            ...
            using (Graphics g = Graphics.FromHdc(_cTrackDc.Hdc))
            {
                ...
                using (GraphicsMode mode = new GraphicsMode(g, SmoothingMode.HighQuality))
                {
                    ...
                    // Draw highlight if values are set.
                    if (ShowHighlight &&
                        (FirstHighlightedFrame >= Minimum &&
                        FirstHighlightedFrame <= Maximum) ||
                        (LastHighlightedFrame >= Minimum &&
                        LastHighlightedFrame <= Maximum))
                    {
                        #region Determine Highlight Rect
                        int highlightLength;
                        int addition;
                        if ((FirstHighlightedFrame <= Maximum && FirstHighlightedFrame >= Minimum) ^
                            ((LastHighlightedFrame <= Maximum && LastHighlightedFrame >= Minimum)))
                        {
                            highlightLength = 1;
                            if (FirstHighlightedFrame <= Maximum && FirstHighlightedFrame >= Minimum)
                            {
                                addition = (int)PosFromValue(FirstHighlightedFrame)
                                                            + ButtonSize.Width / 2;
                            }
                            else
                            {
                                addition = (int)PosFromValue(LastHighlightedFrame)
                                                            + ButtonSize.Width / 2;
                            }
                        }
                        else
                        {
                            highlightLength = (int)(PosFromValue(LastHighlightedFrame) -
                                                        PosFromValue(FirstHighlightedFrame));
                            addition = (int)PosFromValue(FirstHighlightedFrame)
                                                        + ButtonSize.Width / 2;
                        }
                        ...
                        if (Orientation == Orientation.Horizontal)
                        {
                            highlightRect.Width = highlightLength;
                            highlightRect.X += addition;
                        }
                        else
                        {
                            highlightRect.Height = highlightLength;
                            highlightRect.Y += addition;
                        }
                        #endregion

                        using (GraphicsPath hp = CreateRoundRectanglePath(g, highlightRect, 2))
                        {
                            using (LinearGradientBrush highlightBrush = new LinearGradientBrush(
                                buttonRect,
                                Color.FromArgb(120, Color.White),
                                Color.FromArgb(255, this.HighlightColor),
                                (Orientation == Orientation.Horizontal) ? LinearGradientMode.Vertical : LinearGradientMode.Horizontal))
                            {
                                Blend blnd = new Blend();
                                blnd.Positions = new float[] { 0f, .5f, 1f };
                                blnd.Factors = new float[] { .5f, .7f, .3f };
                                highlightBrush.Blend = blnd;

                                g.FillPath(highlightBrush, hp);
                            }
                        }
                    }
                }
                ...
                using (GraphicsMode mode = new GraphicsMode(g, SmoothingMode.HighQuality))
                {
                    using (GraphicsPath gp = CreatePointedRectangularPath(g, flyoutRect, PointDirection.Bottom, 5, 4, offset))
                    {
                        ...
                        using (StringFormat sf = new StringFormat())
                        {
                            ...
                            g.DrawString(data, _flyoutFont, Brushes.Black, flyoutRect, sf);
                        }
                    }
                }
            }
        ...
        private void ScrollChange(ChangeType change, bool decrease)
        {
            ...
            else
            {
                ...
                {
                    if (decrease)
                        count = this.Value - Math.Abs(Maximum - Minimum);
                    else
                        count = this.Value + Math.Abs(Maximum - Minimum);
                }
            }
